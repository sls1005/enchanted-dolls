...
def validate_current_dir():
    if not exists(storage):
        print_err('This command can only be used in a directory generated by "dolls."')
        exit(-1)
...
def link_with(path):
    ...
    if exists(dirname):
        ...
        if os.path.isdir(dirname):
            ...
            kind = 'file'
        print_err('A ' + kind + ' named "' + dirname + '" already exists in the current directory.')
        exit(-1)
    else:
        os.mkdir(dirname)
...
def delete_file_and_link(name):
    ...
    if name in listdir(path):
        ...
        if linked == path + name:
            ...
            print_err('\033[1;36m%s\033[0m does not point to any file in the target directory.' % name)
            exit(-1)
    else:
        print_err('"%s" not found in the target directory.' % name)
        exit(2)
...
if __name__ == '__main__':
    ...
    for i, a in enumerate(argv[1:]):
        if a in ('-h', '--help', 'help'):
            ...
            k = i + 2
            if len(argv) == k:
                print_err('Expect a path after "%s"' % a)
                exit(-1)
            path = argv[k]
            check_if_exists(path)
            link_with(path)
            ...
            k = i + 2
            if len(argv) == k:
                print_err('Expect a name after "%s"' % a)
                exit(-1)
            name = argv[k]
